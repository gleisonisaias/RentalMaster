...
/**
 * Função para obter as datas formatadas para uso nos templates
 */
function getCurrentDateTime() {
  ...
  return {
    dateTimeString: now.toLocaleDateString('pt-BR') + ' ' + now.toLocaleTimeString('pt-BR'),
    shortDate: now.toLocaleDateString('pt-BR'),
    longDate: now.toLocaleDateString('pt-BR', {
      weekday: 'long',
      day: 'numeric',
      month: 'long',
      year: 'numeric'
...
/**
 * Função vazia para manter compatibilidade - não adiciona mais cabeçalho automático
 */
function addHeaderAndFooter(doc: PDFDocumentWithMethods, pageNumber: number) {
  // Função vazia - não faz mais nada
  // Os cabeçalhos e rodapés são controlados pelo modelo através das tags {{DATA_ATUAL}} e {{PAGINA}}
}
...
// Função getCurrentDateTime já está definida acima
...
export async function generateResidentialContractPDF(req: Request, res: Response) {
  try {
    ...
    // Inicializar contador de páginas - a primeira página já é criada automaticamente pelo PDFKit
    let pageNumber = 1;
    
    // Adicionar cabeçalho e rodapé na primeira página
    addHeaderAndFooter(doc, pageNumber);
    
    // Monitorar adição de novas páginas para adicionar cabeçalho e rodapé
    doc.on('pageAdded', () => {
      addHeaderAndFooter(doc, pageNumber);
    });
    ...
    try {
      ...
      if (useTemplate && templateContent) {
        ...
        // Inicializar com fonte padrão
        doc.font('Helvetica').fontSize(10);
        
        // Usar a função de renderização de template HTML, passando o número da página
        // Configurar para processar cores e formatações avançadas
        try {
          renderHTMLTemplate(doc, templateContent, pageNumber);
        } catch (renderError) {
          console.error("Erro ao renderizar HTML:", renderError);
          // Em caso de erro, tentar uma abordagem mais simples
          doc.text(templateContent.replace(/<[^>]*>/g, ''));
        }
        
        // Adicionar campos para assinatura
        // Removido para evitar rodapé automático em templates personalizados: addSignatureFields(doc, owner, tenant);
      } else {
  ...
  try {
    ...
    try {
      ...
      if (useTemplate && templateContent) {
        ...
        // Não incluir assinatura automática para templates personalizados
        } else {
          // Usar formato padrão original
          useTemplate = false;
        }
...
export async function generateContractRenewalPDF(req: Request, res: Response) {
  try {
    ...
    // Adicionar eventos para controlar a numeração de páginas
    let pageNumber = 1;
